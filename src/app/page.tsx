...
export default function Home() {
  ...
  return <main>
    ...
    {phase === "preview" && <section className="stack textbook">
      ...
      <article className="card prompt-card"><p className="eyebrow">CHATGPT LIVE INSTRUCTIONS</p><h2>ロールプレイ指示書</h2><p>場面・難易度・「{practiceMode === "scripted" ? "例文どおり" : "変化あり"}」を反映済みです。</p><ol className="live-steps"><li><b>指示書をコピー</b></li><li><b>場面専用チャットへ貼り付けて送信</b></li><li><b>同じチャットでVoiceを開始</b></li></ol><div className="prompt-preview">{chatGptInstructions(user, scenario, scene, practiceMode)}</div><button className="primary" onClick={copyInstructions}>{copied ? "コピーしました" : "指示書をコピー"}<span>{copied ? "✓" : "□"}</span></button><a className="secondary chatgpt-link" href="https://chatgpt.com/" target="_blank" rel="noreferrer">ChatGPTを開く</a><small>同じ場面は同じチャットで繰り返します。「練習終了」で短い総評と次回の課題が出ます。</small></article>
    </section>}
