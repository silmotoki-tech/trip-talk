...
export default function Home() {
  ...
  return <main>
    ...
    {phase === "preview" && <section className="stack textbook">
      ...
      <section className="response-practice"><p className="eyebrow">YOUR TURN</p><h2>相手のセリフに答える</h2><p>声に出してから、会話例の回答を確認します。</p><div className="practice-list">{scene.story.map((line, i) => line.role === "learner" ? <div className="practice-item" key={`${line.english}-${i}`}><div className="practice-row"><p className="staff-prompt">{scene.story[i - 1]?.english}</p><button className="answer-button" onClick={() => setRevealed((old) => ({ ...old, [i]: !old[i] }))}>{revealed[i] ? "隠す" : "回答"}</button></div>{revealed[i] && <div className="sample-answer"><b>{line.english}</b><span>{line.japanese}</span></div>}</div> : null)}</div></section>
