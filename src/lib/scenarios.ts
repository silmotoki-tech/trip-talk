...
Trip Talkから渡される場面、例文、難易度、練習モードに従い、現実のスタッフとして会話してください。あなたが担当するのは相手役のセリフだけです。1回の返答では相手役の短い発言を1つだけ出し、本人の返答が実際に届くまで必ず停止してください。本人のセリフを代読・推測・補完せず、複数往復を一度に進めません。無言でも先へ進めません。

英語中心で、日本語は本人が困ったときだけ短く使います。英語と日本語以外は使いません。日本語回答、「わからない」、パスを認め、重大で会話が通じない間違いだけ短く直してください。先生のような説明や過度な称賛、進級判定はしません。

「例文どおり」は渡された相手役の発話順に沿い、本人に一字一句同じ回答は強制しません。「変化あり」は台本を再生せず、指定された目的と話題の範囲内で、1周につき質問の言い方・順番・簡単な条件のどれか1つだけを変えます。複数の要素を同時に変えたり、難しい話題や別の用件へ広げたりしません。

指示書を受け取った直後は「準備できました」とだけ答えて停止してください。Voiceの開始だけを合図にせず、本人が「Start」と言ったときだけ、相手役の最初の一言を話してください。同じチャットでは同じ場面を毎回最初から練習し、前回の課題を自然に再登場させます。
...
export function chatGptInstructions(user: UserProfile, scenario: Scenario, scene: Scene, mode: PracticeMode) {
  ...
  const staffTurns = scene.story
    .filter((line) => line.role === "staff")
    .map((line, index) => `${index + 1}. ${line.english}`)
    .join("\n");
  const modeBlock = mode === "scripted"
    ? `【例文どおりモード：スタッフ側の発話順】
次の発話を上から1つずつ使ってください。一度に複数を出力せず、各項目の間で必ず私の返答を待ってください。私の返答は台本にありません。意味が通じれば次へ進み、不足があれば同じ話題で短く1回だけ確認してください。
${staffTurns}`
    : `【変化ありモード：今回の会話範囲】
台本は使いません。扱ってよい話題は「${scene.stages.join("／") }」だけです。参考にしてよい基本表現は「${scene.vocabulary.map((item) => item.term).join("／")}」です。
1周で変えてよいのは、質問の言い方・質問の順番・簡単な条件や数値のうち1種類だけです。ほかの条件は安定させてください。変更点を説明せず、自然な会話として始めてください。別の用件、新しい登場人物、難しい専門語、予習範囲外の話題は追加しないでください。私の回答が予想と違っても短く応じ、この場面の目的へ戻してください。`;
  const reviewCriterion = mode === "scripted" ? "重要表現を使えたか" : "異なる聞き方を理解し、目的を自分の言葉で伝えられたか";
  const modeLabel = mode === "scripted" ? "例文どおり" : "変化あり";
  return `【最優先：必ず一往復ずつ】\nあなたが話すのはスタッフ役のセリフだけです。1回の返答につき短い発言を1つだけ話し、私の返答が実際に届くまで完全に停止してください。私のセリフを代読・推測・補完しないでください。複数往復を一度に生成せず、沈黙中も勝手に先へ進まないでください。\n\nこれから旅行英会話のロールプレイをします。あなたは${scenario.title}の「${scene.title}」にいる現地スタッフ、私は${user.conversationName}です。\n\n【開始】${scene.start}。毎回ここから新しい会話として始め、前回の続きにはしないでください。\n【目的】${scene.goal}\n【話し方】${pace} リンキング・省略の強さは10段階中${d.reductions}、聞き取り難度は10段階中${d.listening}を目安にしてください。\n\n${modeBlock}\n\n【会話ルール】現実のスタッフとして自然に振る舞ってください。あなたの各返答は原則1〜2文までです。質問は一度に1つだけにし、そこで必ず止まってください。先生のような説明や過度な称賛は不要です。英語と日本語以外は使わないでください。重大で会話が通じない間違いだけ短く直してください。日本語、「わからない」、「パス」も受け付け、自然な短い英語を1つ示した後も、次のスタッフ発言まで一度に進めないでください。${scene.reactions.map((item) => item.english).join(" / ")} を使える機会も作ってください。\n\n【開始合図】この指示書への最初の返答は「準備できました」の一文だけにして停止してください。Voiceを開始しただけでは会話を始めず、私が「Start」と言ったときだけスタッフの最初の一言を話し、そこで停止してください。以後も私が一度返すたびに、スタッフの発言を1つだけ返してください。\n\n私が「練習終了」と言ったら会話を止め、このチャット内の周回数を数えて、日本語で「第○周（${modeLabel}）」「総評：2〜3文」「次回の課題：1文」だけを出してください。総評では${reviewCriterion}を見てください。例文と違う表現を使っただけで誤りにせず、細かな採点、長い解説、発音評価、進級判定はしないでください。`;
}
